...
function drawAll() {
  ...
  if (showingTitleScreen) {
    ...
    hiddenCanvasContext.drawImage(canvas, destX, destY, copyW, copyH, srcX, srcY, copyW, copyH);

    canvasContext.drawImage(canvas, srcX, srcY, copyW, copyH, destX, destY, copyW, copyH);
...
function MovingWrapPosition() {
  ...
  this.buildLines = function () {
    ...
    var lines = [];
    lines.push([{ x: this.verts[0].x, y: this.verts[0].y }, { x: this.verts[num_verts - 1].x, y: this.verts[num_verts - 1].y }]);
    for (var j = 1; j < num_verts; j++) {
      lines.push([{ x: this.verts[j - 1].x, y: this.verts[j - 1].y }, { x: this.verts[j].x, y: this.verts[j].y }]);
    }
...
function Ship() {
  ...
  this.buildLines = function () {
    return [[{ x: -16, y: -13 }, { x: 16, y: 0 }], [{ x: 16, y: 0 }, { x: -16, y: 13 }], [{ x: -9, y: -10 }, { x: -9, y: 10 }]];
  };
